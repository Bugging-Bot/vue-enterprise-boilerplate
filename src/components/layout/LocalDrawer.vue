<!-- Shape Information Drawer Component Displays selected shape properties in a collapsible side drawer -->
<template>
  <v-navigation-drawer
    v-model="localDrawer"
    :location="'right'"
  ...
  >
    ...
    <v-divider></v-divider>
    <!-- Workflow Name Input Section (only for builder tab) -->
    <div v-if="currentConfig?.showWorkflowNameInput" class="pa-4">
      <v-text-field
        ...
        @input="handleWorkflowNameChange"
      ></v-text-field>
    </div>

    <!-- Divider and Label for workflow nodes -->
    <div v-if="currentConfig?.showWorkflowNameInput">
      <v-divider></v-divider>
...
const handleWorkflowNameChange = () => {
  // Emit workflow name change event
...
const handleTabChange = (tabValue: string) => {
  ...
  // Reset workflow name when switching tabs
  if (tabValue === 'builder') {
    workflowName.value = ''
  }
}
...
const handleViewChange = (viewName: string) => {
  ...
  // Reset workflow name when view changes
  workflowName.value = '' // this might need to be changed as user might come back to the same view
}
